<table style="margin-left: auto; margin-right: auto">
	<tr>
		<td>Address 1<Span style="color: red">*</Span>:
		</td>
	...
	<tr>
		...
		<td><select id="country" name="Country" style="width: 100%"
			required>
	...
	<tr>
		<td>Zip Code<Span style="color: red">*</Span>:
		</td>
			...
			function() {
				...
				$.get("DropdownServlet", function(data, status) {
					$.each(data, function(index, country) {
						countryDropdown.append($('<option></option>').attr(
								'value', country.countryCode).text(
								country.countryName).attr('data-zipcode',
								country.zipcode));
					});
				});
					...
					$.get("DropdownServlet", {
						...
						$.each(data, function(index, state) {
							stateDropdown.append($('<option></option>').attr(
									'value', state.stateCode).text(
									state.stateName).attr('data-attribute1',
									state.countryCode));
						});
					});
				});
				...
				$("#state").change(
						function() {
							...
							var countryCode = $(this).find(':selected').data(
									'attribute1'); //	Access data attribute for the selected option
							$.get("DropdownServlet", {
								stateCode : stateCode,
								countryCode : countryCode
							}, function(data, status) {
...
<script>
	...
	$("#zipcode").on('input', function() {
		...
		timeout = setTimeout(validateZipcode, 500); // Set a new timeout for 500 milliseconds
	});

	$("#zipcode").on('focusout', function() {
		validateZipcode(); // Perform validation when the field loses focus
	});
						...
						function(data, status) {
							if (data.valid === false) {
								document
										...
										.setCustomValidity(
												"This zipcode is not valid for the selected city. Please enter a valid zipcode.");
